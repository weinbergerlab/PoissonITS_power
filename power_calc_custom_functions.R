...
ts.extract.func<-function(ds2, outcome.name='J12_18',covar.names='none', 
                          nsim, ve.irr, post.start='2010-01-01',overdispersed=TRUE){
  ...
  covar.names2<- c(covar.names,time.vars)
    ...
    covar.names2<- c(time.vars)
  }
  covars2<- paste(covar.names2, collapse="+")
#  form1<-as.formula(paste0(outcome.name, '~', covars2))
  ...
  n.pre.time<-which(ds2$date==post.start)-1  
...
its_func <- function(ds,
                     ...
                     covars=x$covars,
                     post_period=c('2010-01-01', '2014-12-01'),
                     eval_period=c('2012-01-01', '2014-12-01'),
                     seasonN = 12 ,
                     overdispersed=T){
...
est.vax.eff<-  function(x, overdisperse1=F,n.season=12,time_points.select){
  ve<-t(apply(x$preds.stage2, 2,its_func ,time_points=time_points.select,seasonN=n.season, covars=x$covars, overdispersed=overdisperse1))
  return(ve)
}
